...
class HouseForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Make area_of_apartments_total not required
        self.fields['area_of_apartments_total'].required = False
        # Set initial value to 0 for new houses
        if not self.instance.pk:  # If this is a new house
            self.fields['area_of_apartments_total'].initial = 0

    def clean(self):
        cleaned_data = super().clean()
        # Set area_of_apartments_total to 0 if not provided
